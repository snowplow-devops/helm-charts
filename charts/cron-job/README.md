...
| tolerations | list | `[{"operator":"Exists"}]` | Tolerations for the cron job pods. Defaults to tolerating all taints so jobs can be scheduled onto tainted nodes (e.g. Karpenter-provisioned nodes). Set to [] to disable and use default scheduling. |
